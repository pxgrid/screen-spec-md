...
const renderPage = function(tmplMap, pageInfo) {
  const template = pageInfo.screen ? tmplMap.get('spec') : tmplMap.get('index')
  const mdHTML = md2html(pageInfo.mdSource)
  const html = template({
    toRoot: pageInfo.toRoot,
    title: pageInfo.title,
    screen: pageInfo.screen,
    absolutesScreen: pageInfo.absolutesScreen,
    svgCanvas: makeSvgCanvasSrc(pageInfo.dir, pageInfo.screen),
    body: mdHTML,
    updatedAuthorName: pageInfo.updatedAuthorName,
    updatedDate: pageInfo.updatedDate,
    createdAuthorName: pageInfo.createdAuthorName,
